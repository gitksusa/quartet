...
export default function FloatingNav() {
  ...
  return (
    // w-auto h-auto にして、右下のボタン周辺以外は一切タップを邪魔しないように修正
    <div className="fixed bottom-6 right-4 md:right-8 z-50 flex flex-col items-end gap-3 w-auto h-auto pointer-events-none">
      {/* メニューのポップアップ */}
      <div
        ...
        } bg-white/95 backdrop-blur-md border border-[#8e735b]/30 shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-2xl p-4 mb-1 flex flex-col gap-1 w-56 relative z-40 pointer-events-auto`}
      >
      ...
      {/* ボタン部分（ここだけタップできるように pointer-events-auto を指定） */}
      <div className="flex flex-col gap-3 relative z-50 items-end pointer-events-auto">
        {/* ハンバーガーメニューボタン */}
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-12 h-12 bg-white/90 backdrop-blur-md border border-[#8e735b]/30 text-[#8e735b] rounded-full flex items-center justify-center shadow-lg hover:bg-white transition-all duration-300"
        >
        ...
        {/* LINE問い合わせボタン */}
        <a href="#contact" className="w-14 h-14 bg-[#06C755] text-white rounded-full flex flex-col items-center justify-center shadow-xl hover:bg-[#05b34c] hover:scale-105 transition-all duration-300 group">
