...
- **何を追加するか**: 以下を含む統合業務管理ドメイン。① スタッフ管理（個別アカウント・勤怠・給与）、② 物品管理（在庫・経費）、③ 売上・収入管理、④ 経営分析ダッシュボード（客数・単価・施術時間・スタッフ稼働時間等の実データからメニュー増減時の利益影響を予測するシミュレーション機能を含む）、⑤ HP分析ダッシュボード（アクセス数・流入元・滞在時間等をオーナーが自身で確認・HP編集に活かせるセルフサービス型）。電子カルテ的なメニュー構成をイメージし、ロールベースでメニュー項目を出し分ける（オーナーは全項目、権限を付与されたスタッフは許可された項目のみ表示）。
...
- **Authority**: 未定（着手時に `docs/design/` 配下で詳細設計してから確定する）
